...
def compute_expected_profit(buy_cny: float, sell_cny: float, fee_pct: float = 2.5) -> float:
    fee = sell_cny * fee_pct / 100
    return (sell_cny - fee) - buy_cny
...
def compute_score(
    ...
    margin_pct = compute_margin_pct(inputs.buy_cny, inputs.sell_cny, fee_pct=fee_pct)
